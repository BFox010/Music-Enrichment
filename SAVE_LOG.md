...
- 2026-06-05 HEAVY BASS HUMAN OVERLAY — owner reviewed a 316-track shortlist (unlabeled tracks carrying bass-genre cues) in an Excel keep/remove/maybe sheet: 216 keep / 77 remove / 23 maybe. Decisions saved version-controlled at docs/bass_review_2026-06-05.csv. New `scripts/apply_bass_labels.py --apply` layers "Heavy Bass" onto the 216 keeps WITHOUT pruning co-moods (Heavy Bass appended to existing mood_tags; row promoted to mood_source="manual", confidence high → durable, protected from centroid cleanup/suppression). Heavy Bass 484 → 700; centroid rows 1285 → 1070, manual 0 → 216; audit/claude untouched. Idempotent; run as the LAST mood step (after Phase 8 + cleanup_centroid_moods). +6 tests. Recovers the recall the Heavy Bass suppression gave up (centroid is bass-blind). 23 "maybe" rows left unlabeled for a possible later pass.
- 2026-06-05 MOOD CENTROID HARDENING — new `scripts/eval_mood_centroids.py` (reproducible centroid-vs-human reliability report, no audit CSV needed) replaces the manual Excel spot-check. Declarative `CENTROID_MOOD_TREATMENTS` in classify_moods.py (suppress vs gate; `CENTROID_SUPPRESSED_MOODS` now derived from it). Heavy Bass suppressed (feature-inadequate: no bass descriptor → ~10x over-tag in Rock/Indie/Metal). Moody/Slow tempo-gated (<125 / <105) via `apply_centroid_policy()` shared by classifier + cleanup. `scripts/cleanup_centroid_moods.py --apply` scrubbed existing centroid tags in tracks.jsonl (Heavy Bass −464, Moody −239, Slow −146; 4 rows cleared; audit/Claude untouched). Verdict ledger: docs/mood_centroid_decisions.md. +24 tests. Authoritative Phase 6 re-run still pending on owner's machine (needs audit+exportify CSVs); cleanup is idempotent and converges.
- 2026-06-05 SCROBBLE UPDATE — full Last.fm export re-ingested: 13,947 scrobbles (+278), 2,769 unique tracks (+39). 39 new tracks carry play stats but await network-side enrichment.
- 2026-05-30 AI MOOD TAGGING — classified 522/565 Claude-batch tracks (multi-label, owner's calibrated style). Library mood coverage 79.3% → 98.4% (2,687/2,730); 43 left null (skits/intros/interludes/unknowns). Verdicts generated by scripts/write_mood_results.py (classifications keyed by batch index = the version-controlled source of truth) → inputs/claude_mood_results.jsonl (gitignored), applied by Phase 6 as mood_source="claude_batch" (high confidence). Override any track by editing the script + re-running 6→8. tracks.jsonl regenerated.
