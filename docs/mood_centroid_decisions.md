> **Historical — the mechanism here is superseded; the evidence is not.**
>
> This ledger was written against a hand-kept `CENTROID_MOOD_TREATMENTS` list
> that declared per-mood "suppress"/"gate" verdicts. `main` replaced that in
> `cda27fa` ("Gate mood tags on measured precision, not a hand-kept list") with
> `pipeline.evaluate_moods`, which derives the allowlist from k-fold
> cross-validated per-mood precision instead of a maintained table.
>
> Kept because the **reasoning** below still explains *why* the measurements come
> out as they do — above all that the 9 audio features contain no bass
> descriptor, which is why Heavy Bass scores 0.286 precision and is withheld.
> The conclusions were reached by hand here and independently confirmed by
> measurement later; the tables remain the record of how they were first found.
> Do not reintroduce the treatments dict.
